...
def _draw_logo(fig, rect: list, img_path: str, bg_color: str = None):
    """Render a logo image in a figure-level axes at rect=[x,y,w,h]."""
    if not img_path or not os.path.isfile(img_path):
        return
    try:
        img = mpimg.imread(img_path)
        ax  = fig.add_axes(rect)
        ax.set_facecolor(bg_color if bg_color else "none")
        ax.axis("off")
        ax.imshow(img, aspect="equal", interpolation="lanczos", origin="upper")
...
#
#  Header layout:
#    [Left logo]  Title … Section … Doc No.  [Right logo]
#  Footer layout:
#    [Hyperlink]  Generated … Confidential   Page X of Y
# ═══════════════════════════════════════════════════════════════

_LOGO_W = 0.075    # 7.5% figure-width reserved per logo
_HDR_Y  = 0.945
_HDR_H  = 0.055
_STR_H  = 0.008
...
def _draw_chrome(fig, page_num: int, total_pages: int,
    ...
    txt_l = _LOGO_W + 0.005 if has_left  else 0.012
    txt_r = 1 - _LOGO_W - 0.005 if has_right else 0.988

    hdr.text(txt_l, 0.50, REPORT_META["title"],
             va="center", fontsize=9, fontweight="bold",
             color=C["txt_white"], transform=hdr.transAxes)
    hdr.text(0.50, 0.50, section[:72], va="center", ha="center",
    ...
    if has_left:
        _draw_logo(fig,
                   [0.002, _HDR_Y + 0.003, _LOGO_W - 0.004, _HDR_H - 0.006],
                   b["left_logo_path"], bg_color=C["header_bar"])
    if has_right:
        _draw_logo(fig,
                   [1 - _LOGO_W + 0.002, _HDR_Y + 0.003,
                    _LOGO_W - 0.004, _HDR_H - 0.006],
                   b["right_logo_path"], bg_color=C["header_bar"])
...
def _make_cover(pdf, tdms_path, plots_config):
    ...
    # Logos inside navy band
    if has_left:
        _draw_logo(fig, [0.010, 0.820, 0.130, 0.155],
                   b["left_logo_path"], bg_color=C["cover_accent"])
    if has_right:
        _draw_logo(fig, [0.860, 0.820, 0.130, 0.155],
                   b["right_logo_path"], bg_color=C["cover_accent"])
...
def generate_report_simple(
        ...
        output_path:      str,   # Input 1
        x_label:          str,   # Input 2
        y_labels_csv:     str,   # Input 3  comma-separated
        report_title:     str = "",  # Input 4  "" = default
        doc_no:           str = "",  # Input 5  "" = default
        left_logo_path:   str = "",  # Input 6  "" = no logo
        right_logo_path:  str = "",  # Input 7  "" = no logo
        footer_link_text: str = "",  # Input 8  "" = source filename
        footer_link_url:  str = "") -> str:  # Input 9  "" = no link
    """
    Generate PDF — single plot group, each Y = its own subplot.
...
def generate_report_multi(
        tdms_path:        str,   # Input 0
        output_path:      str,   # Input 1
        plots_json:       str,   # Input 2  JSON array
        left_logo_path:   str = "",  # Input 3
        right_logo_path:  str = "",  # Input 4
        footer_link_text: str = "",  # Input 5
        footer_link_url:  str = "") -> str:  # Input 6
    """
    Generate PDF — multiple plot groups from a JSON array.
    plots_json: '[{"x":"Time(Sec)","y":["Normal Load (N)","CoF"]},…]'
    Returns 'SUCCESS: <path>'  or  'ERROR: <message>'.
    """
    try:
